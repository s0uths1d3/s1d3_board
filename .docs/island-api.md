...
> API v1.2.0 · 适用 S1d3 Board ≥ v0.4.0 · 本文档与实现严格同步，以代码为准（实现：`src-tauri/src/island_api.rs`）
...
# {"ok":true,"data":{"version":"1.2.0","port":12935}}
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `text` | string | 显示内容（与弹岛一致，120 字符截断版） |
| `kind` | string | 同 §5.3 |
...
`island.show` 是**全量显示事件流**——凡是真实弹上灵动岛的事件都会广播，不区分来源：
...
被合并丢弃（出现延迟窗口内后者覆盖前者）或被总开关拦截的事件**不会**广播——SSE 严格等于「用户实际看到的岛」。
...
每次**实际弹岛**（与应用内/SSE 同源：复制/粘贴/剪切、任务与便签操作反馈、待办提醒、第三方 API 推送）向每个启用目标投递：
...
| 历史记录 | 每条实际显示事件写入灵动岛历史窗口（用户可回溯），text 为 120 字符截断版 |
| 总开关 | 关闭「灵动岛提示」后：API 仍返回 `204` 但不弹岛、不落历史、**不广播 SSE**；SSE 连接保持 |
...
2. **灵动岛定位为「尽力而为」通知**：可能被合并/顶掉/总开关关闭——关键路径保留系统通知或日志，SSE 确认（收到 `island.show`）仅作为显示回执
...
| SSE 收不到应用自身事件 | 确认事件确实弹上了岛（总开关/延迟合并会吞事件）；重连后不回放历史 |
