...
> API v1.3.0 · 适用 S1d3 Board ≥ v0.4.0 · 本文档与实现严格同步，以代码为准（实现：`src-tauri/src/island_api.rs`）
...
# {"ok":true,"data":{"version":"1.3.0","port":12935}}
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `text` | string \| null | 文本内容（与弹岛一致，120 字符截断版；图片事件为 `null`） |
| `kind` | string | 同 §5.3 |
