...
| 🧠 **智能剪贴板** | 复制内容值得被加工 | 规则分词、AI 加工（OpenAI 兼容 / Anthropic 原生）、模板重组、气泡窗口快捷粘贴 |
...
- **数据**：清空数据库，提供 **5 秒撤回窗口**
- **灵动岛**：复制 / 粘贴时屏幕顶部胶囊反馈，出现延迟与停留时长可自定义，悬停可展开查看完整内容
- **灵动岛 API**：仅监听本机的 HTTP/SSE 端点（端口可配、可选 token），第三方应用 `POST /api/island/show` 即可在灵动岛显示自定义提示（图标类型 / 标题 / 内容 / 停留时长均可指定），接入文档见 [`.docs/island-api.md`](.docs/island-api.md)
